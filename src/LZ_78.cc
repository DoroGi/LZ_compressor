...
void LZ78Compressor::compress(istream &is, ostream &os) {
  ...
  while (is.get(c)) {
    ...
    if (dictionary.count(s) == 0) { 
      ...
      // The +1 are needed because the index 0 should never be used.
      // (0,a) means that a is NOT in the dictionary! 
...
void LZ78Compressor::decompress(istream &is, ostream &os) {
  ...
  // The last codeword is only 16 bit long (max). We will handle it at the end.
  int index_lenght = 0;
  while (bit_buffer.size()>index_lenght) {
